...
# Structure:
# - user_prefs: List of dicts [{"sun_elev": float, "preferred_lux": float}]
# - room_curves: Dict mapping brightness % to lux yield {"50": 12.5}
# - media_prefs: List of historical brightness percentages [20, 25, 20]
# - late_night_prefs: List of historical brightness percentages [10, 15, 10]
...
# Structure: {room_id: [(timestamp, lux), ...]} — max 5 entries per room.
LUX_HISTORY = {}

# Minimum brightness percentage that produces visible light output.
# Most Zigbee/LED drivers produce no visible output below ~5%.
MIN_VISIBLE_PCT = 5
...
# Includes intermediate states (paused, idle, buffering) so Media Learning path isn't broken during active watch sessions.
...
def safe_get_state(entity_id, default=0):
    ...
    """
    try:
        # Handle attribute requests like 'light.living_room.brightness'
...
def get_smoothed_lux(room_id, lux_sensor, max_readings=5):
    ...
    sensor values and returning the median. This filters transient spikes caused by
    direct sunlight hitting the sensor during sunset/sunrise.
    """
    global LUX_HISTORY
    ...
    # Trim to max_readings
    if len(LUX_HISTORY[room_id]) > max_readings:
        LUX_HISTORY[room_id] = LUX_HISTORY[room_id][-max_readings:]

    # Discard readings older than 30 minutes (stale data from a previous session)
    cutoff = now - 1800
    ...
    # Return the median of stored readings (median is more spike-resistant than mean)
...
def parse_entity_list(raw_input):
    ...
    Converts literal strings like "['media_player.tv', 'media_player.soundbar']" into a proper Python list.
    This prevents Blueprint multi-select inputs from silently breaking the media/presence checks.
    """
...
def get_expected_lux(curve, pct, default_ratio=1.0):
    ...
    if pct <= 0: return 0.0
    if not curve: return float(pct) * float(default_ratio) # User configurable fallback ratio
    points = sorted([(int(k), float(v)) for k, v in curve.items()])
    if not points: return float(pct) * float(default_ratio) # User configurable fallback ratio

    # C4 Guard against zero division if points[0][0] is 0
...
def get_pct_for_lux(curve, target_artificial_lux, default_ratio=1.0):
    ...
    if not curve: return min(100, int(target_artificial_lux / float(default_ratio))) # User configurable fallback ratio
    points = sorted([(int(k), float(v)) for k, v in curve.items()])
    if not points: return min(100, int(target_artificial_lux / float(default_ratio)))

    # C4 Guard against zero division if points[0][1] is 0
    ...
    for i in range(len(points) - 1):
        p1_pct, p1_lux = points[i]
        p2_pct, p2_lux = points[i+1]
        # Ensure we don't divide by zero if two points have identical lux
...
def load_data():
    """
    Runs automatically when Pyscript starts up or is reloaded.
    Loads the saved AI learning data from the local JSON file into the global memory dictionary.
    """
    global LEARNING_DATA
    try:
        # Use task.executor to prevent blocking the HA event loop during file I/O
        if task.executor(os.path.exists, DATA_FILE):
            ...
            if data_str:
                ...
                # Merge loaded persistent data into our global memory dictionary
                for k in LEARNING_DATA:
                    if k in loaded: 
                        LEARNING_DATA[k] = loaded[k]
                log.info(f"SmartLighting: Loaded learning data from JSON for {len(LEARNING_DATA.get('user_prefs', {}))} rooms.")
        else:
            log.info("SmartLighting: No existing JSON data found. Starting fresh.")
    except Exception as e:
        log.warning(f"SmartLighting: Failed to load JSON data. ({e})")

    # Announce that Pyscript has loaded and is ready to accept service calls
    state.set("pyscript.passable_smart_light_engine_ready", "on", new_attributes={"startup_time": time.time()})

def save_data():
    """
    Saves the global LEARNING_DATA dictionary back to the local JSON file.
    C1: Creates a deep copy snapshot to prevent RuntimeError during concurrent thread execution.
    """
    try:
        p = pathlib.Path(DATA_FILE)
        task.executor(p.parent.mkdir, parents=True, exist_ok=True)
        # Create a deep copy snapshot of global memory before thread serialization
        data_copy = copy.deepcopy(LEARNING_DATA)
        data_str = json.dumps(data_copy, indent=4)
        task.executor(p.write_text, data_str)
        log.info("SmartLighting: 💾 Successfully saved updated AI preferences to JSON.")
    except Exception as e:
        log.error(f"SmartLighting: Failed to save JSON data - {e}")

def get_circadian_temp(min_temp=2700, max_temp=5500):
    """
    Calculates the ideal color temperature (Kelvin) based on the sun's current elevation.
    Maps elevation (-10 to +50 degrees) proportionally to the provided min/max temp range.
    """
    try:
    ...
    # Clamp elevation to a reasonable daytime swing
...
def passable_smart_light_engine_event_wrapper(trigger_type=None, event_type=None, context=None, **kwargs):
    ...
    This prevents Home Assistant from throwing 'Unknown Action' errors on restart
    before Pyscript has fully loaded.
    """
...
def passable_smart_light_engine(
    ...
    **kwargs
    ...
    """
    The core control logic. Called by the Home Assistant Blueprint upon any relevant state change.
    Evaluates context (bypasses, presence, media, late night) and takes appropriate lighting action.
    """
    if not room_id: return
    log.info(f"SmartLighting [{room_id}]: Engine triggered by '{trigger_id}'")
    
    # Initialize missing data structures for new rooms dynamically to prevent KeyError
    global LEARNING_DATA
    ...
    def is_late_night():
        """Evaluates if the current time or state is within the defined late night window."""
        if str(late_night_enabled).lower() not in ["true", "on"]: return False
        
        # Option 1: Evaluate single entity state mode
        ...
        # Option 2: Prioritize dynamic entity overrides, fall back to hardcoded times
        ...
        try:
            # H3: Extract time portion if input_datetime contains full date (e.g., "2026-07-21 22:00:00")
            ...
            # Handle standard windows vs overnight windows (e.g., 22:00 to 06:00)
    ...
    def enforce_monotonic_curve(curve):
        """Ensures a curve is strictly monotonic (lux always increases with percentage)."""
    ...
    def auto_calibrate_task(r_id, l_ent, lux_sens, from_pct, pct_target, duration):
        """
        Asynchronous background task. Waits for the light transition to physically
        complete, then waits for the next sensor update to capture the final stable lux.
        Uses Continuous Delta Learning.
        C2: Uses try...finally to guarantee sensor_stale flag cleanup.
        """
        ...
        try:
            ...
            # Only calibrate at night (elevation < 0) when sun variability doesn't pollute the lux yield data
            if elev >= 0:
                return 
            ...
            # Step 1: Wait for the light transition to physically complete (plus 2s buffer)
            task.sleep(duration + 2)

            # Step 2: Wait for the NEXT lux sensor report to arrive
            try:
                current_lux_str = str(safe_get_state(lux_sens, 0))
                log.info(f"SmartLighting [{r_id}]: Fade finished. Waiting for final Lux report...")
                task.wait_until(state_trigger=f"{lux_sens} != '{current_lux_str}'", timeout=120)
            except task.TimeoutError:
                log.warning(f"SmartLighting [{r_id}]: Lux sensor timed out after fade. Using current reading.")

            # Double check the light didn't change while we were waiting
            ...
            # Apply the correction if the delta is logically sound
    ...
    def turn_on_light(pct, transition_time=None):
        ...
        # Apply circadian color temp if enabled and supported by hardware
        ...
        # Trigger background calibration to learn room yield
        if pct > 0:
    ...
    def turn_off_light(transition_time=5):
        """Standardized function to turn off lights and update tracking states."""
        current_bright_val = safe_get_state(f"{light_entity}.brightness", 0)
        from_pct = int((float(current_bright_val) / 255) * 100) if current_bright_val else 0
    ...
    def async_set_override(entity, state_str):
        """Executes service call to set helper entity state."""
    ...
    def set_manual_override(state_str):
        """Helper to safely toggle the optional dashboard input_boolean override tracker."""
    ...
    # Handle light-off here, before bypass checks and before path routing.
    # H2: Set manual_off flag so motion/lux don't immediately re-trigger light
    if trigger_id == "light_change":
        ...
        if is_off:
            ...
            # H2: Set manual_off timestamp flag for presence_timeout duration
    ...
    # Auto-Sync: C3 Synchronous reset on timer expiry
    ...
    if bypass_freeze_entities:
        for entity in parse_entity_list(bypass_freeze_entities):
            # C3: Ignore freeze bypass if entity is manual_override_entity and timer is expired/inactive
    ...
    # Gather Current Context (Presence & Media) using list parser
    ...
    # ------------------------------------------
    # PATH 1: EVALUATION PATH
    # ------------------------------------------
    # Triggered by presence, media changes, lux changes, bypass state changes, or 5-min heartbeat.
    eval_triggers = ["presence_on", "media_change", "lux_change", "bypass_change", "heartbeat"]
    if trigger_id in eval_triggers:
        # C2: Halt evaluation if waiting for stale sensor, with 15-second safety fallback check
        if safe_get_state(light_entity, "off") == "on" and safe_get_state(f"pyscript.sensor_stale_{room_id}", "false") == "true":
            ...
            if trigger_id == "lux_change":
                ...
                # Fallback: auto-expire stale flag if locked for over 15 seconds
                state.set(f"pyscript.sensor_stale_{room_id}", "false")
            else:
                return

        # H2: Respect manual_off flag until expired or room re-entered
        ...
        # Priority 1: User Override exists - halt completely so we don't fight the user
        if override_time > time.time(): return 

        # Priority 2: Room is vacant - let the separate timeout handler turn it off
        ...
        # SCENARIO A: Media is Active
        if media_active:
            current_lux = get_smoothed_lux(room_id, lux_sensor)

            # 1. H4: Dynamic AI weighting for media preferences
        ...
        # SCENARIO C: Standard Daytime Logic (Lux Yield Targeting)
        else:
            current_lux = get_smoothed_lux(room_id, lux_sensor)
            ...
            # Retrieve historical user overrides for this room
            ...
            if prefs:
                ...
                if total_w > 0:
                    ...
                    n_count = len(prefs)
                    w_learned = min(1.0, n_count / 10.0)
                    # H4: Dynamic N/10 AI weight curve
    ...
    # ------------------------------------------
        ...
        log.info(f"SmartLighting [{room_id}]: Presence Timeout Fired.")
        if media_active: return
        
        # Clear active manual off flag when vacancy timeout occurs
        state.set(manual_off_key, "0")

        # M2: Only clear override and turn off light if manual override timer has expired
        if override_time <= time.time():
            state.set(override_key, "0")
            set_manual_override("off")
            turn_off_light(5)
        else:
            log.info(f"SmartLighting [{room_id}]: Vacancy timeout ignored — manual override still active.")
    ...
    # ------------------------------------------
        ...
        # Context-based override detection
        ...
        # H1: Trajectory-bounded automation echo guard with post-fade grace window
        last_auto_end = float(safe_get_state(f"pyscript.last_auto_{room_id}", 0))
        ...
        # 30-second post-automation grace window to account for delayed Zigbee/Hue mesh reporting
        ...
        if within_grace_period and last_target_pct is not None and str(last_target_pct).lower() not in ["none", "unknown"]:
            try:
                # 8% tolerance accounts for hardware dimming curve & quantization rounding (e.g., 76% target -> 70% hardware level)
        ...
        if not is_manual:
            log.debug(
                f"SmartLighting [{room_id}]: Ignored — "
        ...
        log.info(f"SmartLighting [{room_id}]: ✅ Manual override detected via {source} at {current_pct}%")

        # Check if max-brightness-from-off should skip override timer
        ...
        if str(ignore_max_brightness_override).lower() in ["true", "on"]:
            ...
            if was_off and is_max:
                log.info(f"SmartLighting [{room_id}]: Max brightness from off — skipping override timer per config.")
        ...
        if media_active:
            def _save_media():
                ...
                if current_pct >= 95 and int(media_seed_pct or 20) < 50:
                    log.info(f"SmartLighting [{room_id}]: Rejecting media learning — {current_pct}% is too far from seed {media_seed_pct}%")
                    return
            ...
            def _save_late_night():
                ...
                if current_pct >= 95 and int(late_night_pct or 0) < 50:
                    log.info(f"SmartLighting [{room_id}]: Rejecting late-night learning — {current_pct}% is too far from seed {late_night_pct}%")
                    return
            ...
            def _save_daytime():
                ...
                try:
                    ...
                    log.info(f"SmartLighting [{room_id}]: ⏳ Waiting up to 120s for lux sensor to stabilize...")
                    task.wait_until(state_trigger=f"{lux_sensor} != '{current_lux_str}'", timeout=120)
                    lux_after = float(safe_get_state(lux_sensor, 0))

                    if lux_after <= 0:
                        log.warning(f"SmartLighting [{room_id}]: Rejecting user pref — lux_after is {lux_after} (sensor may be dead)")
                        return

                    smoothed = get_smoothed_lux(room_id, lux_sensor)
                    if abs(lux_after - smoothed) > 20:
                        log.info(f"SmartLighting [{room_id}]: Lux reading {lux_after} looks contaminated (smoothed={smoothed}). Using smoothed value.")
                        lux_after = smoothed
                    ...
                    # M3: Filter daylight flare contamination by evaluating net artificial contribution
                    ...
                    log.warning(f"SmartLighting [{room_id}]: Lux sensor timed out. Discarding manual preference.")
...
def passable_smart_light_engine_reset(room_id=None, reset_type="all"):
    ...
    global LEARNING_DATA
    if not room_id:
        log.warning("SmartLighting: Reset called without room_id — resetting ALL rooms.")
        ...
        for key in targets:
            if key in LEARNING_DATA:
                ...
                log.info(f"SmartLighting: 🗑️ Reset {key} for all rooms")
        save_data()
        return
    ...
    for key in targets:
        if key in LEARNING_DATA and room_id in LEARNING_DATA[key]:
            LEARNING_DATA[key][room_id] = [] if "prefs" in key else {}
            log.info(f"SmartLighting [{room_id}]: 🗑️ Reset {key}")

    save_data()
    log.info(f"SmartLighting [{room_id}]: Learning data reset complete ({reset_type}).")